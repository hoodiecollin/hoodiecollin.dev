...
bun run resume:pdf # regenerate public/collin-kokotas-resume.pdf (needs Chrome)
...
- **Resume** — the canonical résumé is typed data in `lib/resume.ts`, the single
  source of truth for **both** the `/resume` page and the downloadable PDF, so
  they can't drift. `scripts/build-resume-pdf.ts` renders that data through
  headless Chrome to `public/collin-kokotas-resume.pdf` (also gitignored,
  regenerated in `prebuild`). CI runs on GitHub's `ubuntu-22.04` runner, which
  ships Chrome, so the PDF is rebuilt fresh on every deploy; locally it needs
  Chrome (skips gracefully in `bun dev` if absent — run `bun run resume:pdf` to
  refresh). The job-search slant variants stay in the separate resume workspace.
...
scripts/              build-search-index.ts + build-resume-pdf.ts (both prebuild)
```
