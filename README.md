...
- **Search index** (`public/search-index.json`) is generated by the `prebuild`
  ...
  parse, so each contributes an explicit entry in that script and lands in the
  palette's **Pages** group (`SearchDoc.group`).
...
```
app/                 routes (landing, /writing, /writing/[slug], /resume, /ai-pm-playbook) + layout/globals
components/           chrome (header/footer/nav/search), mdx/, ui/ (shadcn)
content/writing/      MDX posts
lib/                  site config, resume data, playbook data, mdx loader, search, toc, util helpers
